...
function run_swift {
  ...
  xcodebuild clean build -project TeaXml.xcodeproj -scheme "TeaXml-Package" -sdk "macosx" -destination "platform=OS X,arch=x86_64" -configuration Debug ONLY_ACTIVE_ARCH=NO test
  xcodebuild test -project TeaXml.xcodeproj -scheme "TeaXml-Package" -sdk "macosx" -destination "platform=OS X,arch=x86_64" -configuration Debug ONLY_ACTIVE_ARCH=NO test || return 126
  cd ../
...
function run_python {
  ...
  coverage run --source="./alibabacloud_tea_xml" -m pytest tests/test_* || return 126
...
function run_python2 {
  ...
  coverage run --source="./alibabacloud_tea_xml" -m pytest tests/ || return 126
